...
#define IRTX_38K()					/* TX && NEC/AEHA: Set IR sub-carrier to 38kHz */
#define IRTX_40K()					/* TX && SONY: Set IR sub-carrier to 40kHz */
#define IRTX_ON()					/* TX: IR sub-carrier on */
#define IRTX_OFF()					/* TX: IR sub-carrier off */
#define IRTX_TST()					/* TX: Check status of IR sub-carrier (true:on) */

#define CAPT_STA()					/* RX: Set capture on IR start (left blanked if it can capture on both edge) */
#define CAPT_STP()					/* RX: Set capture on IR stop (left blanked if it can capture on both edge) */
#define CAPT_TST()	(!PINDESC_GET(PIN_IR))				/* RX: Check which edge triggered the capture event (true:IR start) */
#define	CAPT_VAL()	TCNT0				/* RX: Get the captured timer value */
#define CAPT_ENA()					/* RX: Enable capture interrupt */
#define CAPT_DIS()					/* RX && TX: Disable capture interrupt */

#define COMP_ENA(n)					/* Always: Enable compare interrupt n count after now */ do { \
    OCR0A = TCNT0+(n); \
    TIFR = _BV(OCF0A); \
    TIMSK |= _BV(OCIE0A); \
} while (0)
#define COMP_DIS()	TIMSK &= ~_BV(OCIE0A);				/* Always: Disable compare interrupt */
#define COMP_NXT(n)	OCR0A += (n)				/* TX: Increase compare register by n count */

/* Counter clock rate and register width */
#define T_CLK		64000				/* Always: Timer tick period [ns] */
#define	irtmr_t		uint8_t			/* Always: Integer type of timer counter */

/* Definitions of interrupt service functions */
#define	ISR_COMPARE()	ISR(TIMER0_COMPA_vect)	/* Always: Timer compare match ISR */
#define	ISR_CAPTURE()	ISR(PCINT0_vect)	/* RX: Timer input capture ISR */
#define CALLBACK_TX()	IR_ontx()			/* TX: Callback function on end of transmission */
